...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        ModuleToBuild = 'PSVVX'
        ModuleVersion = '0.0.1'
        ModuleWebsite = 'https://github.com/zloeber/psvvx'
        ModuleCopyright = "(c) $((get-date).Year.ToString()) Zachary Loeber. All rights reserved."
        ModuleLicenseURI = 'https://github.com/zloeber/psvvx/LICENSE.md'
        ModuleTags = 'Polycom,VVX,API,REST' -split ','
        ModuleAuthor = 'Zachary Loeber'
        ModuleDescription = 'PowerShell module for Polycom VVX devices'
        ...
        # You should not actually enter this key here but should manually enter it in the PSVVX.buildenvironment.json file
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "PSVVX.buildenvironment.json"
